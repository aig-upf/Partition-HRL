...
class ManagerA2CPCIntraRewards(AbstractManager):
    ...
    def _train_simulate(self, env, train_episode=None):
        ...
        while not done:
            ...
            # update the option
            intra_reward = self.compute_intra_reward(o_r_d_i[0]["option"])
            current_option.update_option(o_r_d_i, action, correct_termination, intra_reward, train_episode)
    ...
    def compute_intra_reward(self, new_state):
        ...
        if number_next_options == 0:
            return 0

        else:
            return max([option.get_critic_value(new_state) for option in self.option_list[:number_next_options]])