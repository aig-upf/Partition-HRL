...
class AgentDQNMontezuma(AbstractAgent):
    ...
    def act(self, train_episode=None):
        ...
        act_values = self.model.predict(self.current_state)  # [[action 1, action 2]]
    ...
    def update_agent(self, o_r_d_i, action, train_episode=None):
        if train_episode:
            self.memory.append((self.current_state, action, o_r_d_i[1], self.my_hash(o_r_d_i[0]), o_r_d_i[2]))
            self.current_state = np.array([self.my_hash(o_r_d_i[0])])

        self.score += self.compute_total_score(o_r_d_i)

    def reset(self, current_state):
        self.current_state = np.array([self.my_hash(current_state)])

    def compute_total_reward(self, o_r_d_i):
